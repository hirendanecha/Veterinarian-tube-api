...
var Appointments = function (appointment) {
  ...
  this.veterinarianProfileId = appointment.veterinarianProfileId;
  this.veterinarianName = appointment.veterinarianName;
...
Appointments.findAndSearchAll = async (
  ...
  let whereCondition = `${
    search ? `a.veterinarianName LIKE '%${search}%'` : ""
  }`;
  ...
  const searchData = await executeQuery(
    `SELECT a.id, a.veterinarianName, a.appointmentDateTime, a.createdDate,a.profileId,a.isCancelled,p.FirstName,p.LastName,p.ProfilePicName,p.Username FROM appointments as a left join profile as p on p.ID = a.profileId  ${
      whereCondition ? `WHERE ${whereCondition} ` : ""
...
Appointments.getPractitionerAppointments = async (id) => {
  const query =
    "select a.*,p.Username,p.FirstName,p.LastName,p.ProfilePicName from appointments as a left join profile as p on p.ID = a.profileId where a.isCancelled = 'N' and a.veterinarianProfileId = ?";
  const values = [id];
...
Appointments.getUserAppointments = async (id) => {
  const query =
    "select a.*,p.Username,p.FirstName,p.LastName,p.ProfilePicName from appointments as a left join profile as p on p.ID = a.veterinarianProfileId where a.isCancelled = 'N' and a.profileId = ?";
  const values = [id];
  const appointmentList = await executeQuery(query, values);
  if (appointmentList) {
    return appointmentList;
