...
exports.communityApproveEmail = async (profileId, isApprove) => {
  ...
  if (userData) {
    ...
    let msg = "";
    if (isApprove === "Y") {
      msg = `VeterinarianTube has approved your Veterinarian application.`;
    } else {
...
exports.cancelAppointmentNotificationMail = async (id, veterinarianName) => {
  const query =
    "select u.Email,p.FirstName,p.LastName,p.Username from users as u left join profile as p on p.UserID = u.Id where p.ID =?";
  const values = [id];
...
exports.sendAppointmentMailToUser = async (data) => {
  ...
  const query1 =
    "select u.Email,p.FirstName,p.LastName,p.Username from users as u left join profile as p on p.UserID = u.Id where p.ID =?";
  const values1 = [data.veterinarianProfileId];
...
const getIcalObjectInstance = async (
  ...
  cal.createEvent({
    ...
    url: url, // 'event url',
    summary: "Request video call with Veterinarians",
    organizer: {
      // 'organizer details'
      name: name,
      email: email,
